...
The corpse-persistence check additionally executes the radius/total overlap model that previously under-evicted, and protects stock Off passthrough, the 4096/24/48 protocol bounds, the shared Gameplay setting, new-profile-only Modern seeding, the strict multiplayer gore gate, and the disabled EchoPatch game/world hooks. These are intentionally static ownership/order and script-model tests, not substitutes for the documented live corpse-budget, visual-sever, rapid-fire, gore-disabled, and save/reload acceptance pass.
